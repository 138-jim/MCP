...
def register_deliver_tools(mcp: FastMCP, state: ServerState):
    ...
    @mcp.tool()
    @resolve_tool
    def resolve_set_render_format_and_codec(format: str, codec: str) -> str:
        """Set render format and codec directly.

        Args:
            format: Render format identifier (e.g. 'mp4', 'mov').
            codec: Codec identifier (e.g. 'H264', 'H265').
        """
        proj = _get_project(state)
        if proj.set_current_render_format_and_codec(format, codec):
            return f"Set render format={format}, codec={codec}"
    ...
    def resolve_start_render(job_ids: str = "") -> str:
        """Start rendering. Optionally specify comma-separated job IDs.

        Args:
            job_ids: Comma-separated job IDs. If empty, renders all jobs.
        """
        proj = _get_project(state)
        if job_ids:
            ids = [j.strip() for j in job_ids.split(",") if j.strip()]
            proj.start_rendering(*ids)
            return f"Rendering started for jobs: {', '.join(ids)}"
