...
- **`resolve_mcp`** — MCP server exposing 162 tools for LLM-driven control of Resolve
...
- [Python Library Guide](#python-library-guide)
  ...
  - [Graph (Color Node Graph)](#graph-color-node-graph)
...
The MCP server lets LLMs (Claude, etc.) control DaVinci Resolve through 162 tools. It runs as a stdio process that any MCP-compatible client can connect to.

The server also exposes a `resolve://guide` resource containing an AI workflow guide. MCP clients that support resources can read it for tips on effective tool usage.
...
The `Graph` wrapper provides full control over the color node graph for a timeline item, including node state, caching, LUTs, and grade management.
...
# Nodes
graph.get_num_nodes()
graph.get_node_label(1)
graph.get_node_enabled(1)
graph.set_node_enabled(1, False)
graph.get_tools_in_node(1)

# Node cache: 0=None, 1=Smart, 2=On
graph.get_node_cache_mode(1)
graph.set_node_cache_mode(1, 2)  # cache on
...
Manage color groups for organizing clips by camera or scene.

```python
groups = project.get_color_group_list()
group = project.add_color_group("A-Cam")
group.get_name()            # "A-Cam"
group.set_name("B-Cam")

# Timeline clips in a group
clips = group.get_clips_in_timeline(timeline)

# Pre/post-clip node graphs for the group
pre_graph = group.get_pre_clip_node_graph()
post_graph = group.get_post_clip_node_graph()
pre_graph.get_num_nodes()

# Delete
project.delete_color_group(group)
```
...
See [MCP_TOOLS.md](MCP_TOOLS.md) for a complete reference of all 162 MCP tools with parameters and descriptions.
